---
name: reviewer2
description: Write a simulated "Reviewer 2" referee report on a paper before Jake submits it --- a panel of distinct referee personas plus a champion, a self-audit that grounds every objection in the paper's text, and a prioritized revision plan. Use when asked for a referee report, a Reviewer 2 report, a pre-submission review, red-teaming of a paper, or what a referee at a named journal (APSR, AJPS, JOP, JASA, AOAS, Biometrika, Annals, Nature, Science, PNAS) would say. Not for answering real reviewers after a decision --- that is the review-response skill.
---

# Reviewer 2: the pre-submission referee report

This skill gives an AI assistant instructions for writing a simulated referee
report --- a "Reviewer 2" report --- on a paper Jake Bowers is preparing to
submit. The goal is to find every real weakness in the paper before a real
referee does, and to hand Jake a prioritized plan for fixing them. The scope is
any paper headed for peer review: political science journals (APSR, AJPS, JOP),
statistics journals (JASA, AOAS, Biometrika, Annals of Statistics), and
general-science journals (Nature, Science, PNAS).

Related instructions that apply alongside this skill:

- the global `CLAUDE.md` --- writing style, ASCII discipline, intellectual
  engagement. The report you write must obey its writing rules.
- `CLAUDE_CODING.md` (imported by the global `CLAUDE.md`) --- coding
  preferences, for papers with code or replication packages.
- the `math` skill --- mathematical-statistics work. Section 5 of this file
  reuses its checklist for the technical read.
- the `verify-citations` skill --- citation verification. The self-audit in
  section 4.6 borrows its method and retargets it from citations to objections.
- the `review-response` skill --- the mirror image of this one. That one helps
  Jake answer real reviewers; this one helps him anticipate them. The revision
  plan this skill produces (Step 7) is the seed of the response memo that one
  governs.
...
citations. `verify-citations` exists because the second failure is quiet and
...
- **Ask for a different paper.** Reviewing the paper the referee wishes Jake had
  ...
  the `review-response` skill. A suggestion to do a different project is a scope
  note, not a flaw.
- **Demand gratuitous citation.** "You must cite X" where X is the reviewer's own
  ...
  `verify-citations` bar: X is real, X exists, and X is genuinely relevant to the
...
paper claims, in the paper's own terms. The `math` skill calls this stating the
...
- **The target of inference.** If causal or inferential: what is the paper after?
  ...
  estimator? (the `math` skill 6.4--6.5.) In the Fisher-Rosenbaum tradition, a
...
policies go stale. Apply the `verify-citations` discipline: verify from a real page
or say you did not.
...
here as a fabricated citation is in `verify-citations`.
...
- A "you ignored X (year)" objection must clear the `verify-citations` bar: X is
...
`verify-citations` verification method retargeted from citations to objections. Re-read
...
- **Kill fabricated omissions.** For every "you ignored X" or "you should cite X,"
  confirm X is real, exists, and is relevant (the `verify-citations` bar). Drop the
  ones that fail.
...
- **Decline with reason.** Objections that are real concerns but that Jake can
  ...
  referee. These seed the response memo governed by the `review-response` skill.
...
Run these during Step 3. Reuse the `math` skill rather than restating it --- this
...
paper for not reporting Gamma, is asking for a different paper. The `math` skill
...
- **Estimand / estimator / estimate kept distinct** (the `math` skill 6.4). Is the
  target defined before the estimator? Does the paper estimate the thing it
  defined?
- **Identification before estimation** (the `math` skill 6.5). Does the estimand
...
- **Finite-sample vs. asymptotic** (the `math` skill 6.6). Does the paper claim a
...
11. Does every "you ignored X" name a real, relevant work (the `verify-citations`
    bar)?
