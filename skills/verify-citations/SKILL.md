...
description: Verify every citation in a paper, memo, grant, talk, or referee response against external sources (Crossref, OpenAlex, arXiv, ORCID) before it leaves Jake's desk. Use when asked to verify citations, check a bibliography, or confirm references are real; when a .bib file is in scope before submission, posting, or circulation; and whenever an AI assistant drafted or edited any reference. Bundles a script that automates the per-entry API checks (levels L1-L4).
---

# Bibliography verification

This skill gives an AI assistant instructions for verifying the bibliography of a paper, memo, talk, grant, or referee response before it leaves the desk. The scope is every citation that appears in a piece of writing intended for an external audience.

The global `CLAUDE.md` (writing style, ASCII discipline) applies alongside this skill. The `math` skill states the citation rules at the level of authoring (its `references/verification.md`, section 13); this skill states the rules at the level of verification.

A bundled script, `scripts/verify_bib.py`, automates the per-entry lookups of sections 4--5 and drafts the verification log of section 9. Run it first; section 12 says what it covers and what remains manual.
...
- Never paraphrase a citation in a way that strengthens the cited claim (see the math skill, `references/verification.md`, section 13.2).
...
2. For each entry, run the verification protocol (section 5) using the sources in section 4 in the order: DOI -> Crossref -> OpenAlex -> arXiv -> ORCID -> Scholar -> publisher page. The bundled script `scripts/verify_bib.py` runs this loop for every entry and writes a draft log; treat its output as the starting point and handle every entry it flags by hand.
...
First choice: the bundled script.

```
python3 scripts/verify_bib.py paper.bib -o bib_verification.md
```

(Paths are relative to this skill directory; use the absolute path to the script when running from a project repository.) The script parses the `.bib`, queries Crossref and OpenAlex (and arXiv for preprint ids) in the polite pools, compares normalized metadata field by field, assigns each entry a level (L0--L4) and a failure category (F1--F6) per sections 3 and 7, and writes the verification-log skeleton of section 9. What it does not do, and what therefore remains manual: Pass D (L5 --- locating cited claims inside the works), Pass E (in-text consistency), Pass F (the plausibility read), judgment on books, working papers, websites, and datasets beyond a search attempt, and every decision about a discrepancy. When the script cannot run (no network, no `python3`), fall back to the manual protocol using the tools below.
